...
class DesktopPet {
  constructor() {
    ...
    this.state = 'idle'; // 'idle', 'walk', 'run', 'float', 'sleep', 'drag', 'knead'
  ...
  update(mouseX, mouseY) {
    ...
    if (hasKeyInput) {
      ...
      const isAutoBehavior = document.getElementById('action-select').value === 'auto';
      ...
      if (this.isAntiGravity) {
        ...
        if (isOnFloor) {
          if (pettingMeter > 8) {
            ...
            this.vy = 0;
            if (Math.random() < 0.08) {
              particles.push(new Particle(this.x + this.width / 2 + (Math.random() - 0.5) * 10, this.y + 10, 'heart'));
            }
          } else if (typingHeat > 0) {
            ...
            const isMouseClose = Math.abs(mouseX - (this.x + this.width / 2)) < 850;
            ...
            if (this.state === 'walk' || this.state === 'run') {
              if (Math.abs(this.vx) < 0.2) {
                this.vx = (this.facing === 'right' ? 1 : -1) * (this.state === 'run' ? 2.2 : 1.2);
              }
            } else {
              this.vx = 0;
            }
  ...
  draw(ctx, skinFilter) {
    ...
    if (isSpriteSheetLoaded && spriteSheetImage) {
      ...
      drawFallbackPanda(ctx, 0, 0, this.width, this.height, this.state, this.facing, this.animFrameIndex + this.animTick);
    }
...
// ==========================================
function drawFallbackPanda(ctx, x, y, width, height, state, facing, frame) {
  ctx.save();
  ...
  if (state === 'run' || state === 'knead') {
    ctx.fillRect(4*s, 24*s, 14*s, 12*s);
  } else {
    ctx.fillRect(6*s, 28*s + breathY, 12*s, 16*s);
  }
  
  if (state === 'run' || state === 'knead') {
    ctx.fillRect(46*s, 22*s, 14*s, 12*s);
  } else {
    ctx.fillRect(46*s, 28*s + breathY, 12*s, 16*s);
  }
  
  ctx.fillStyle = '#f8fafc';
  ctx.fillRect(14*s, 26*s + breathY, 36*s, 22*s - breathY);
  
  ctx.fillStyle = '#1e293b';
  ctx.fillRect(10*s, 4*s + breathY*0.5, 12*s, 12*s);
  ctx.fillRect(42*s, 4*s + breathY*0.5, 12*s, 12*s);
  
  ctx.fillStyle = '#f8fafc';
  ctx.fillRect(14*s, 10*s + breathY*0.5, 36*s, 22*s);
  
  ctx.fillStyle = '#1e293b';
  ctx.fillRect(18*s, 18*s + breathY*0.5, 8*s, 8*s);
  ctx.fillRect(38*s, 18*s + breathY*0.5, 8*s, 8*s);
  
  if (state === 'float') {
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(20*s, 20*s + breathY*0.5); ctx.lineTo(24*s, 24*s + breathY*0.5);
    ctx.moveTo(24*s, 20*s + breathY*0.5); ctx.lineTo(20*s, 24*s + breathY*0.5);
    ctx.moveTo(40*s, 20*s + breathY*0.5); ctx.lineTo(44*s, 24*s + breathY*0.5);
    ctx.moveTo(44*s, 20*s + breathY*0.5); ctx.lineTo(40*s, 24*s + breathY*0.5);
    ctx.stroke();
  } else if (state === 'sleep') {
    ctx.strokeStyle = '#94a3b8';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(19*s, 22*s + breathY*0.5); ctx.lineTo(25*s, 22*s + breathY*0.5);
    ctx.moveTo(39*s, 22*s + breathY*0.5); ctx.lineTo(45*s, 22*s + breathY*0.5);
    ctx.stroke();
  } else {
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(21*s, 19*s + breathY*0.5, 3*s, 3*s);
    ctx.fillRect(41*s, 19*s + breathY*0.5, 3*s, 3*s);
  }
...
window.addEventListener('mousemove', (e) => {
  ...
  if (isOverPetHead && !pet.isDragging) {
    ...
    if (Math.abs(dx) > 3) {
      ...
      if (currentDirection !== lastWiggleDirection && lastWiggleDirection !== 0) {
...
function gameLoop() {
  ...
  if (typingHeat > 70) {
    if (Math.random() < 0.15) {
      particles.push(new Particle(pet.x + pet.width / 2 + (Math.random() - 0.5) * 20, pet.y, 'steam'));
    }
  }
  ...
  let skinFilter = 'none';
  if (typingHeat > 70) {
    // Red color tint during Overheat
    skinFilter = 'sepia(1) saturate(5) hue-rotate(-50deg) brightness(0.95)';
  } else {
    switch (currentSkin) {
      case 'gold':
        skinFilter = 'hue-rotate(40deg) saturate(2.2) brightness(1.1)';
        break;
      case 'rose':
        skinFilter = 'hue-rotate(300deg) saturate(1.8) brightness(1.05)';
        break;
      case 'ruby':
        skinFilter = 'hue-rotate(345deg) saturate(3) brightness(0.9)';
        break;
      case 'emerald':
        skinFilter = 'hue-rotate(110deg) saturate(2) brightness(0.95)';
        break;
    }
  }
  ...
  const timerText = (pomodoroState !== 'none') ? `⏱️ Pomodoro (${formatTime(pomodoroRemaining)})` : "";
